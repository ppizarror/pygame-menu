...
class Node:
    ...
    def update(
        ...
        if is_visited != "unchanged":
            assert isinstance(is_visited, bool), (
...
class MazeApp:
    ...
    def __init__(self, width: int = 8, rows: int = 75, margin: int = 0) -> None:
        ...
        self._rows = rows
        self._screen_width = (
            self._rows * (self._width + self._margin) + self._margin * 2
        )
    ...
    def _run_solver(self) -> None:
        ...
        if self._visualize:
            ut.set_pygame_cursor(pygame_menu.locals.CURSOR_NO)
        if solver_type == 0:
            self._path_found = self._dijkstra(
                self._grid, self._start_point, self._end_point
            )
            self._algorithm_run = "dijkstra"
        elif solver_type == 1:
            self._path_found = self._dijkstra(
                self._grid, self._start_point, self._end_point, astar=True
            )
            self._algorithm_run = "astar"
        elif solver_type == 2:
            self._path_found = self._xfs(
                self._grid, self._start_point, self._end_point, x="d"
            )
            self._algorithm_run = "dfs"
        elif solver_type == 3:
            self._path_found = self._xfs(
                self._grid, self._start_point, self._end_point, x="b"
            )
    ...
    def _better_prim(
        self, mazearray: _MazeType | None = None, start_point: _Point2 | None = None
    ) -> _MazeType:
        """
    ...
    def _recursive_division(
        ...
        if chamber_width < 3:
            ...
            for y in range(chamber_height):
                self._grid[chamber_left + x_divide][chamber_top + y].update(
                    nodetype="wall"
                )
        ...
        if chamber_height < 3:
            ...
            for x in range(chamber_width):
                self._grid[chamber_left + x][chamber_top + y_divide].update(
                    nodetype="wall"
                )
    ...
    def _random_terrain(self, num_patches: int | None = None) -> None:
        ...
        for patch in range(num_patches + 1):
            ...
            neighbour_cycles = 0
            centre_point = (
                random.randrange(1, self._rows - 1),
                random.randrange(1, self._rows - 1),
            )
            patch_type = "mud"
            ...
            while len(terrain_nodes) > 0:
                ...
                if (
                    self._grid[node[0]][node[1]].nodetype != "start"
                    and self._grid[node[0]][node[1]].nodetype != "end"
                ):
    ...
    def _update_path(self) -> bool | _MazeType:
        ...
        if self._algorithm_run == "dijkstra":
            ...
            path_found = self._dijkstra(
                self._grid, self._start_point, self._end_point, astar=True
            )
        elif self._algorithm_run == "dfs":
            path_found = self._xfs(
                self._grid, self._start_point, self._end_point, x="d"
            )
    ...
    def _get_neighbours(
        self, node: _Point2, max_width: int | None = None
    ...
    def _dijkstra(
        ...
        # Print timings
        print(
            f"Program finished in {time_taken:.4f}s after checking {num_visited}"
    ...
    def _xfs(
        self, mazearray: _MazeType, start_point: _Point2, goal_node: _Point2, x: str
    ) -> bool:
        """
        ...
        while True:
            ...
            for event in events:
                ...
                # Turn off all mouse drags if mouse Button released
                    ...
                    self._mouse_drag = self._drag_end_point = self._drag_start_point = (
                        False
                    )
                ...
                # Moves the mouse
                    ...
                    if not left:
                        self._mouse_drag = self._drag_end_point = (
                    ...
                    # Move the start point
                        ...
                        if self._grid[row][column].nodetype == "blank":
                            self._grid[self._start_point[0]][
                                self._start_point[1]
                            ].update(nodetype="blank", is_path=False, is_visited=False)
                    ...
                    # Move the end point
                        ...
                        if self._grid[row][column].nodetype == "blank":
                            self._grid[self._end_point[0]][self._end_point[1]].update(
            ...
            self._grid[self._start_point[0]][self._start_point[1]].update(
                nodetype="start"
            )
