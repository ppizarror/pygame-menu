...
__all__ = ["DropSelect", "DropSelectManager"]
...
class DropSelect(Widget):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            isinstance(scrollbar_slider_pad, NumberInstance)
            and scrollbar_slider_pad >= 0
        )
        assert isinstance(scrollbar_thick, int) and scrollbar_thick >= 0
        assert (
            isinstance(selection_box_border_width, int)
            and selection_box_border_width >= 0
        )
        assert isinstance(selection_box_height, int) and selection_box_height >= 1
        assert (
            isinstance(selection_box_text_margin, int)
            and selection_box_text_margin >= 0
        )
        assert isinstance(selection_box_width, int) and selection_box_width >= 0
        assert isinstance(selection_infinite, bool)
        assert (
            isinstance(selection_option_border_width, int)
            and selection_option_border_width >= 0
        )
        ...
        selection_option_selected_bgcolor = assert_color(
            selection_option_selected_bgcolor
        )
        selection_option_selected_font_color = assert_color(
        ...
        if selection_option_font_size is not None:
            assert (
                isinstance(selection_option_font_size, int)
                and selection_option_font_size > 0
            )
        ...
        self._selection_box_margin = (
            int(selection_box_margin[0]),
    ...
    def _apply_font(self) -> None:
        ...
        h = int(
            title_render[1]
            + self._selection_box_inflate[1] / 2
        ...
        self._option_font = get_font(
            self._selection_option_font_style["name"],
            self._selection_option_font_style["size"],
        )
        if self._selection_box_width == 0:
            f = self._render_option_string(self._placeholder)
            h = self._render_string(
                self._title, self.get_font_color_status()
            ).get_height()
            self._selection_box_width = int(
                f.get_width()
    ...
    def _make_selection_drop(self) -> DropSelect:
        ...
        if not self.configured:
            raise RuntimeError(
                f"{self.get_class_id()} must be configured before creating selection drop"
            )
        ...
        for opt_id in range(len(self._items)):
            ...
            btn.set_background_color(color=self._selection_box_bgcolor)
            btn.set_border(
            ...
            btn.set_padding(padding=self._selection_option_padding)
            ...
            if self._selection_option_left_space and not (
                ...
                dh = int(
                    btn.get_height(apply_padding=False)
                    * self._selection_option_left_space_height_factor
                )
                ...
                btn._padding_transform = (
                    prev_pad_t[0],
                    prev_pad_t[1],
                    prev_pad_t[2],
                    prev_pad_t[3] + dh + m[0] + m[1],
                )
        ...
        for btn in self._option_buttons:
            max_width = max(
                max_width, btn.get_width() - self._selection_option_border_width
            )
        ...
        self._drop_frame.set_background_color(color=self._selection_box_bgcolor)
        ...
        if total_height > 0:
            ...
            border_w = (
                self._selection_box_border_width if total_height != max_height else 0
            )
            self._drop_frame.set_attribute(
                "height", max_height + add_scrollbar - border_w
            )
        ...
        if self._drop_frame is not None:
            ...
            if not self._open_middle:
                if self._open_bottom:
                    self._drop_frame.set_position(
                        x + self._title_size[0],
                        y
                        + self._title_size[1]
                    ...
                    self._drop_frame.set_position(
                        x + self._title_size[0],
                        y
                        - self._drop_frame.get_attribute("height")
            ...
            if self._placeholder_add_to_selection_box:
                placeholder_button: Button = self._drop_frame.get_attribute(
                    "placeholder_button"
                )
    ...
    def _render(self) -> bool | None:
        ...
        scroll_v: int = 0
        menu_height: int = (
            0 if self._menu is None else self._menu.get_height(widget=True)
        )
        ...
        current_rect_bg.y += int(
            (self._selection_box_inflate[1]) / 2 + vi + self._selection_box_margin[1]
        )
        current_rect_bg.width = self._selection_box_width
        current_rect_bg = current_rect_bg.inflate(
            (
                int(self._selection_box_inflate[0]),
                int(self._selection_box_inflate[1] + 2 * vi),
            )
        )
        ...
        arrow = pygame.Rect(
            ...
            int(
                self._selection_box_arrow_margin[2]
        ...
        if self._drop_frame is not None and self._scrollarea is not None:
            assert self.get_scrollarea().get_world() is not None, (
                "scrollarea world is None. The widget have been appended to the Menu?"
            )
        ...
        arrow_up = (
            (
                int(arrow.right - w + h / 2 - h / 16),
                int(arrow.centery - h / 6 - h / 20),
            ),
            (
                int(arrow.right - w + h / 2 + h / 4 - h / 16),
                int(arrow.centery + h / 4 - h / 20),
            ),
        ...
        self._surface.blit(
            title, (0, int(self._selection_box_inflate[1] / 2 + delta_title_height))
        )
        pygame.draw.rect(self._surface, self._selection_box_bgcolor, current_rect_bg)
        pygame.draw.rect(
            self._surface,
            self._selection_box_border_color,
            current_rect_bg,
            self._selection_box_border_width,
        )
        ...
        if len(self._items) > 0:
            pygame.draw.polygon(
                self._surface, self._selection_box_arrow_color, arrow_right_pos
            )
    ...
    def update_items(self, items: list[tuple[Any, ...]] | list[str]) -> None:
        ...
        if (
            self._index != -1
            and len(items) != len(self._items)
            or (self._index + 1) > len(items)
        ):
            self._index = -1
    ...
    def get_focus_rect(self) -> pygame.Rect:
        ...
        if self.active:
            rect.width = self._selection_box_width
            rect.x += self._title_size[0]
            rect.height += self._drop_frame.get_attribute(
                "height"
            ) - self._drop_frame.get_attribute("extra_margin")
            if not self._open_bottom:
                rect.y -= self._drop_frame.get_attribute(
                    "height"
                ) - self._drop_frame.get_attribute("extra_margin")
            if self._open_middle:
    ...
    def _drop_frame_scrolling(self) -> bool:
        ...
        """
        if (
            self._drop_frame is None
            or self._drop_frame.get_scrollarea(inner=True) is None
        ):
            return False
    ...
    def update(self, events: EventVectorType) -> bool:
        ...
        for event in events:
            ...
            if (
                keydown
                and self._ctrl.move_down(event, self)
                or joy_hatmotion
            ...
            # Right button
                ...
                keydown
                and self._ctrl.move_up(event, self)
                or joy_hatmotion
            ...
            # Press enter
                ...
                if not (
                    self.active and (not self._close_on_apply and self._index != -1)
                ):
                    self._toggle_drop()
                return True
            ...
            # Close the selection
                ...
                self._ctrl.escape(event, self) or self._ctrl.back(event, self)
            ):
                if self.active:
                    self._toggle_drop()
                return True
            ...
            # Click on dropselect; don't consider the mouse wheel (button 4 & 5)
                ...
                and self._menu is not None
                and not self._drop_frame_scrolling()
            ):
                event_pos = get_finger_pos(self._menu, event)
                if self._drop_frame.get_rect(
                    apply_padding=False, to_real_position=True
            ...
            # Click on dropselect; don't consider the mouse wheel (button 4 & 5)
                ...
                if rect.collidepoint(*event_pos):
                    ...
                    dist = mouse_x - (
                        topleft + self._title_size[0]
                    )  # Distance from title
                    if dist > 0:  # User clicked the options, not title
                        self._toggle_drop()
                        return True
                else:
                    if self.active and not self.get_focus_rect().collidepoint(
                        *event_pos
                    ):
                        self._toggle_drop()
                        return True
