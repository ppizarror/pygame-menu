...
class DropSelectMultiple(DropSelect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            isinstance(selection_option_selected_box_border, int)
            and selection_option_selected_box_border > 0
        )
    ...
    def _render_option_string(self, text: str) -> pygame.Surface:
        ...
        if (
            ...
            or self._max_selected != 0
            and len(self._selected_indices) == self._max_selected
        ):
    ...
    def _get_current_selected_text(self) -> str:
        ...
        if self._selection_placeholder_format == DROPSELECT_MULTIPLE_SFORMAT_LIST_COMMA:
            ...
            self._selection_placeholder_format
            == DROPSELECT_MULTIPLE_SFORMAT_LIST_HYPHEN
        ):
            return self._placeholder_selected.format("-".join(list_items))

        elif isinstance(self._selection_placeholder_format, str):
            return self._placeholder_selected.format(
                self._selection_placeholder_format.join(list_items)
            )
            ...
            try:
                ...
                raise ValueError(
                    "selection placeholder function receives only 1 "
        ...
        raise ValueError(
            f'invalid selection placeholder format type "{self._selection_placeholder_format}"'
        )
    ...
    def _process_index(self) -> None:
        ...
        if self._index == -1:
            ...
            if (
                ...
                or len(self._selected_indices) < self._max_selected
            ):
    ...
    def _make_selection_drop(self) -> DropSelectMultiple:
        ...
        for btn in self._option_buttons:
            ...
            off = deco.add_rectangle(
                x - dh - self._selection_option_left_space_margin[1],
                y + (total_height - dh) / 2,
                dh,
                dh,
                self._selection_option_selected_box_color,
                self._selection_option_selected_box_width,
                use_center_positioning=False,
            )
            on = deco.add_rectangle(
                x - dh - self._selection_option_left_space_margin[1],
                y + (total_height - dh) / 2,
                dh,
                dh,
...
class DropSelectMultipleManager(AbstractWidgetManager, ABC):
    ...
    def dropselect_multiple(
        ...
        **kwargs,
        ...
        selection_option_active_bgcolor = kwargs.pop(
            "selection_option_active_bgcolor", (188, 227, 244)
        )
        selection_option_active_font_color = kwargs.pop(
            "selection_option_active_font_color", (0, 0, 0)
        )
        ...
        selection_option_selected_bgcolor = kwargs.pop(
            "selection_option_selected_bgcolor", (142, 247, 141)
        )
        selection_option_selected_box = kwargs.pop(
            "selection_option_selected_box", True
        )
        selection_option_selected_box_border = kwargs.pop(
            "selection_option_selected_box_border", self._theme.widget_box_border_width
        )
        selection_option_selected_box_color = kwargs.pop(
            "selection_option_selected_box_color", self._theme.widget_box_arrow_color
        )
        selection_option_selected_box_height = kwargs.pop(
            "selection_option_selected_box_height", 0.5
        )
        selection_option_selected_box_margin = kwargs.pop(
            "selection_option_selected_box_margin",
            (
                0,
                self._theme.widget_box_arrow_margin[1],
